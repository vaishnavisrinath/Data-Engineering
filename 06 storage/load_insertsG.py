...
def getSQLcmnds(rowlist):
    ...
    for row in rowlist:
        valstr = row2vals(row)
        cmd = f"INSERT INTO {StagingTableName} VALUES ({valstr});"
        cmdlist.append(cmd)
    return cmdlist
...
def createTable(conn):
    ...
    with conn.cursor() as cursor:
        ...
        """)
        cursor.execute(f"CREATE UNLOGGED TABLE {StagingTableName} (LIKE {TableName});")
        print(f"Created {TableName} and {StagingTableName}")
...
def load(conn, icmdlist):
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute(f"INSERT INTO {TableName} SELECT * FROM {StagingTableName};")
        elapsed = time.perf_counter() - start
        print(f'Finished Loading. Elapsed Time: {elapsed:0.4} seconds')

    conn.commit()  # Commit the transaction after loading all data
