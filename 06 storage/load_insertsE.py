...
def createTable(conn, deferConstraints=True):
...
def main():
    ...
    if CreateDB:
        createTable(conn, deferConstraints=True)
        load(conn, cmdlist)
        createTable(conn, deferConstraints=False)  # Create constraints and indexes after loading data
    else:
        load(conn, cmdlist)
